...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	if(mode == 'info') {
		...
		s += '<div class="c altw"><div  id="messagetext"><p>' + msg + '</p></div></div>';
		s += '<p class="o pns">' + (leftmsg ? '<span class="z xg1">' + leftmsg + '</span>' : '') + (showconfirm ? '<button id="fwin_dialog_submit" value="true" class="bluesmallbutton"><strong>'+confirmtxt+'</strong></button>' : '');
		s += mode == 'confirm' ? '<button id="fwin_dialog_cancel" value="true" class="whitesmallbutton" onclick="hideMenu(\'' + menuid + '\', \'dialog\')"><strong>'+canceltxt+'</strong></button>' : '';
		s += '</p>';
	}
...
function simulateSelect(selectId, widthvalue) {
	...
	for(var i = 0; i < selectObj.options.length; i++) {
		...
		li.k_lable = selectObj.options[i].label;
		if(selectObj.options[i].selected) {
			defaultopt = selectObj.options[i].innerHTML;
			defaultv = selectObj.options[i].value;
			defaultt = selectObj.options[i].label;
		...
		li.onclick = function() {
			if($(selectId + '_ctrl').innerHTML != this.innerHTML) {
				...
				$(selectId).options[0] = new Option(this.k_lable, this.k_value);
	...
	selectObj.options[0]= new Option(defaultt, defaultv);
...
function showTopLink() {
	...
	if(ft){
		...
		if (BROWSER.ie && BROWSER.ie < 7) {
			scrolltop.style.top = viewPortHeight - scrollHeight - 180 + 'px';
		}
		/*if (scrollHeight < -100) {
